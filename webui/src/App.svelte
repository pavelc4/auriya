<script>
    ...
    async function loadSystemInfo() {
        ...
        if (!res1 || res1.error) {
            ...
            if (modTime && !isNaN(modTime)) {
                ...
                if (diff < 3600)
                    timeStr = `Updated ${Math.floor(diff / 60)}m ago`;
                else if (diff < 86400)
        ...
        if (res2 && !res2.error) {
            ...
            const battery =
                parts[5] && !isNaN(parts[5]) ? `${parts[5]}%` : "Unknown";
        ...
        try {
            ...
            if (ratesRes && !ratesRes.error && !ratesRes.startsWith("ERR")) {
                ...
                if (start !== -1) {
                    supportedRefreshRates.set(
                        JSON.parse(ratesRes.substring(start)),
                    );
                }
            }
        } catch (e) {}
    ...
    let currentTheme = "auriya";

    function toggleTheme() {
        const html = document.documentElement;
        const current = html.getAttribute("data-theme");
        const next = current === "light" ? "auriya" : "light";
