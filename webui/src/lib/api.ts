...
declare global {
	interface Window {
		...
		ksu?: unknown;
		$auriya?: unknown;
		$auriyaFile?: { exists: (path: string) => boolean; read: (path: string) => string; write: (path: string, content: string) => void };
		$auriyafile?: { exists: (path: string) => boolean; read: (path: string) => string; write: (path: string, content: string) => void };
	}
}
...
class AuriyaWX extends WXClass {
	...
	get files(): AuriyaFile | null {
		return (typeof window !== 'undefined' && this.fileToken) ? (window[this.fileToken] as AuriyaFile | undefined) ?? null : null;
	}

	async runCommand(cmd: string, cwd: string | null = null): Promise<string | CommandResult> {
		...
		try {
			...
			return errno === 0 ? stdout.trim() : { error: stderr } as CommandResult;
		} catch (e) {
	...
	async readFile(path: string): Promise<string | CommandResult> {
		if (this.files && typeof this.files.read === 'function') return this.files.read(path);
		return await this.runCommand(`cat "${path}"`);
	}

	async writeFile(path: string, content: string): Promise<string | CommandResult> {
