pub const SOCKET_PATH: &str = "/dev/socket/auriya.sock";
pub const MODULE_PATH: &str = "/data/adb/modules/auriya";
pub const CONFIG_DIR: &str = "/data/adb/.config/auriya";
pub const SETTINGS_FILE: &str = "/data/adb/.config/auriya/settings.toml";
pub const GAMELIST_FILE: &str = "/data/adb/.config/auriya/gamelist.toml";
pub const LOG_FILE: &str = "/data/adb/auriya/daemon.log";
pub const SERVICE_SCRIPT: &str = "/data/adb/modules/auriya/service.sh";
